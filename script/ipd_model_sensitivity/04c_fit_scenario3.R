...
#omega_F_13 := omega_F_7 (propagated joint MVN, ipd_fit posterior)
#the Stan parameter name `log_rr_N_post` is basically rr_N_13
#rr_N_pre = rr_N_7 is propagated from carriage_fit's joint MVN for the pre-PCV13 era 1999-2009
...
stan_data <- c(common_stan_data, list(
  scenario = 3L,
  #all three PCV13 parameters use the same wide brackets. In scenario 3
  #only log_rr_N_post is active (informative Beta(7, 2) prior). The other
  ...
  #log_delta_F_7 / log_omega_F_7). Wide brackets are cheap because the
...
#rr_N_post at the Beta(7, 2) mean (= 7/9) on log scale.
init_fun <- make_init_fun(scenario = 3L, active_log_init = log(7 / 9))

mod <- get_pcv13_model()
ipd_fit_scen <- do.call(mod$sample, c(list(data = stan_data, seed = SEED_BASE + 3L, refresh = 50, init = init_fun), mcmc_args))

ipd_fit_scen$save_object(file = "ipd_fit_scenario3.rds")
...
print(ipd_fit_scen$summary(c("rr_N_post")))
