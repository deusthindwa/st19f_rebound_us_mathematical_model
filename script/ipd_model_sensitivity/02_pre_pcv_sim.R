...
simulate_pre_pcv_seed <- function(n_burn_years = 40, verbose = FALSE, pop_99 = NULL) {
  ...
  for (yi in seq_len(n_burn_years)) {
    ...
    if (verbose && (yi %% 10 == 0 || yi == n_burn_years)) { message(sprintf("  R seed burn-in year %d", yi))}
  }
  state
}
...
if (sys.nframe() == 0) {
  ...
  #saveRDS(seed_state, file = here::here("results", "ipd_model_sensitivity", "seed_state.rds"))
}
