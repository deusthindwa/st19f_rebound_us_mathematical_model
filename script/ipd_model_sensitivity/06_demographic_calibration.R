...
#file = here::here("results", "ipd_model_scenarios", "demographic_model.rds")
file = here::here("demographic_model.rds")
)
...
p_overall
#ggsave(here::here("output", "ipd_model_scenarios", "plot_demog_overall.png"), p_overall, width = 9, height = 5, dpi = 150)
...
sim_long <- 
  ...
  mutate(year = as.integer(year)) %>%
  pivot_longer(-year, names_to = "age", values_to = "pop") %>%
  mutate(type = "Model-simulated")

bar_df <- bind_rows(obs_long, sim_long) %>%
  mutate(age  = factor(age, levels = age_labels), 
         type = factor(type, levels = c("Observed", "Model-simulated")))

p_bars <- 
  ggplot(bar_df, aes(x = year, y = log(pop), fill = type)) +
  geom_col(position = position_dodge(width = 0.85, preserve = "single"), width = 0.75) +
  facet_wrap(~ age, scales = "free_y") +
  scale_y_continuous(labels = scales::comma) +
  scale_x_continuous(breaks = seq(1999, 2019, by = 2)) +
  scale_fill_manual(values = c("Observed" = "grey40", "Model-simulated" = "#d95f02")) +
  labs(title = "Age-group population: observed vs model-simulated", x = "Year", y = "Population log_count", fill = NULL) +
  theme_bw() +
  theme(legend.position = "bottom", axis.text.x = element_text(angle = 30, hjust = 1))

p_bars
#ggsave(here::here("output", "ipd_model_scenarios", "plot_demog_by_age.png"), p_bars, width = 11, height = 6, dpi = 150)
