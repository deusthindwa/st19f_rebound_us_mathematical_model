...
for (k in 2:13){
  ipdAfitI[k] <- (ipdAfit_all[k]/abc_sites_pop[k-1]) * 1000000
}
...
ccr <-
  dplyr::bind_cols(
    ...
    data_frame(name2 = c('V_gp1', 'V_gp2', 'V_gp3', 'V_gp4', 'F_gp1', 'F_gp2', 'F_gp3', 'F_gp4', 'N_gp1', 'N_gp2', 'N_gp3', 'N_gp4'),
               value2 = c(344616.2, 1679662, 5369201, 32018339, 349319.9, 1705055, 5460586, 32568640, 348512.7, 1700693, 5444883, 32474079))) %>%
  dplyr::mutate(invas = value/value2)

ccry <-
  dplyr::bind_cols(
    ipdAfitI %>% dplyr::filter(yearc == 2010) %>% tidyr::pivot_longer(V_gp1:N_gp4),
    data_frame(name2 = c('V_gp1', 'V_gp2', 'V_gp3', 'V_gp4', 'F_gp1', 'F_gp2', 'F_gp3', 'F_gp4', 'N_gp1', 'N_gp2', 'N_gp3', 'N_gp4'),
