...
source(here::here("script", "POP", "scenarios", "01_preprocessData_scenario.R"))
obs_traj <- ipdAfitI %>% slice_head(., n = 21)
...
if (scenario == 1){
  parset <- rio::import(here::here("results", "POP", "lhs_parset_projection.csv"))
  
} else if (scenario == 2){
  parset <- rio::import(here::here("results", "POP", "lhs_parset_project_efficacy.csv"))
  
} else if (scenario == 3){
  parset <- rio::import(here::here("results", "POP", "lhs_parset_project_duration.csv"))
  
} else if (scenario == 4){
  parset <- rio::import(here::here("results", "POP", "lhs_parset_project_competition.csv"))
  
} else if (scenario == 5){
  parset <- rio::import(here::here("results", "POP", "lhs_parset_modefit_efficacy.csv"))
  
} else if (scenario == 6){
  parset <- rio::import(here::here("results", "POP", "lhs_parset_modefit_duration.csv"))
  
} else if (scenario == 7){
  parset <- rio::import(here::here("results", "POP", "lhs_parset_modefit_competition.csv"))
...
if (scenario == 1){
  pred_traj <- base::readRDS(here::here("results", "POP", "saved_ipdRun_projection.rds"))
  
} else if (scenario == 2){
  pred_traj <- base::readRDS(here::here("results", "POP", "saved_ipdRun_project_efficacy.rds"))
  
} else if (scenario == 3){
  pred_traj <- base::readRDS(here::here("results", "POP", "saved_ipdRun_project_duration..rds"))
  
} else if (scenario == 4){
  pred_traj <- base::readRDS(here::here("results", "POP", "saved_ipdRun_project_competition.rds"))
  
} else if (scenario == 5){
  pred_traj <- base::readRDS(here::here("results", "POP", "saved_ipdRun_modefit_efficacy.rds"))
  
} else if (scenario == 6){
  pred_traj <- base::readRDS(here::here("results", "POP", "saved_ipdRun_modefit_duration.rds"))
  
} else if (scenario == 7){
  pred_traj <- base::readRDS(here::here("results", "POP", "saved_ipdRun_modefit_competition.rds"))
...
if (scenario == 5) { #model fit of reduced efficacy
  rio::export(data_frame(data.frame(XX$GOF_cases)), here::here("results", "POP", "logLik_efficacy.csv"))
  
} else if (scenario == 6){
  rio::export(data_frame(data.frame(XX$GOF_cases)), here::here("results", "POP", "logLik_duration.csv"))
  
} else if (scenario == 7){
  rio::export(data_frame(data.frame(XX$GOF_cases)), here::here("results", "POP", "logLik_competition.csv"))
...
if (scenario == 1){
  rio::export(posteriorEst, file = here("output", "POP", "estimates_projection.csv"))
  
} else if (scenario == 2){
  rio::export(posteriorEst, file = here("output", "POP", "estimates_project_efficacy.csv"))
  
} else if (scenario == 3){
  rio::export(posteriorEst, file = here("output", "POP", "estimates_project_duration.csv"))
  
} else if (scenario == 4){
  rio::export(posteriorEst, file = here("output", "POP", "estimates_project_competition.csv"))
  
} else if (scenario == 5){
  rio::export(posteriorEst, file = here("output", "POP", "estimates_modefit_efficacy.csv"))
  
} else if (scenario == 6){
  rio::export(posteriorEst, file = here("output", "POP", "estimates_modefit_duration.csv"))
  
} else if (scenario == 7){
  rio::export(posteriorEst, file = here("output", "POP", "estimates_modefit_competition.csv"))
...
#save all plots
if (scenario == 1){
  ggsave(here::here("output", "POP", "projections.png"), plot = post_trajPlot, width = 20, height = 10, unit = "in", dpi = 300)
  
} else if (scenario == 2){
  ggsave(here::here("output", "POP", "project_efficacy.png"), plot = post_trajPlot, width = 20, height = 10, unit = "in", dpi = 300)
  
} else if (scenario == 3){
  ggsave(here::here("output", "POP", "project_duration.png"), plot = post_trajPlot, width = 20, height = 10, unit = "in", dpi = 300)
  
} else if (scenario == 4){
  ggsave(here::here("output", "POP", "project_competition.png"), plot = post_trajPlot, width = 20, height = 10, unit = "in", dpi = 300)
  
} else if (scenario == 5){
  ggsave(here::here("output", "POP", "modefit_efficacy.png"), plot = post_trajPlot, width = 20, height = 10, unit = "in", dpi = 300)
  
} else if (scenario == 6){
  ggsave(here::here("output", "POP", "modefit_duration.png"), plot = post_trajPlot, width = 20, height = 10, unit = "in", dpi = 300)
  
} else if (scenario == 7){
  ggsave(here::here("output", "POP", "modefit_competition.png"), plot = post_trajPlot, width = 20, height = 10, unit = "in", dpi = 300)
...
#extract for plotting F only
if (scenario == 5){
  rio::export(posteriorDS, here::here('results', 'POP', 'modefit_efficacy_plotF.csv'))

} else if (scenario == 6){
  rio::export(posteriorDS, here::here('results', 'POP', 'modefit_duration_plotF.csv'))

} else if (scenario == 7){
  rio::export(posteriorDS, here::here('results', 'POP', 'modefit_competition_plotF.csv'))
...
#plot the F categories
plotF <-
  dplyr::bind_rows(rio::import(here::here('results', 'POP', 'modefit_efficacy_plotF.csv')) %>% dplyr::mutate(strat = 'H1: Reduced direct effectiveness'),
                   rio::import(here::here('results', 'POP', 'modefit_duration_plotF.csv')) %>% dplyr::mutate(strat = 'H2: Reduced duration of protection'),
                   rio::import(here::here('results', 'POP', 'modefit_competition_plotF.csv')) %>% dplyr::mutate(strat = 'H3: Reduced co-colonisation risk')) %>%
  dplyr::filter(stg == 'F') %>%
  dplyr::mutate(strat = factor(strat, levels = c('H1: Reduced direct effectiveness', 'H2: Reduced duration of protection', 'H3: Reduced co-colonisation risk')),
                agegp = factor(agegp, levels =c('<1y', '1-4y', '5-17y', '18+y')))

obsF <- 
  dplyr::bind_rows(observedDS %>% dplyr::mutate(strat = 'H1: Reduced direct effectiveness'),
                         observedDS %>% dplyr::mutate(strat = 'H2: Reduced duration of protection'),
                         observedDS %>% dplyr::mutate(strat = 'H3: Reduced co-colonisation risk')) %>%
  dplyr::filter(stg == 'F') %>%
  dplyr::mutate(strat = factor(strat, levels = c('H1: Reduced direct effectiveness', 'H2: Reduced duration of protection', 'H3: Reduced co-colonisation risk')),
                agegp = factor(agegp, levels =c('<1y', '1-4y', '5-17y', '18+y')))
...
sensitivity_pop <-
  ggplot() +
  geom_point(data = obsF, aes(x = yearc, y = m), shape = 1, stroke =1.5) + 
  geom_line(data = plotF, aes(x = yearc, y = m, color = timex, group = timex), size = 0.8) +
  geom_ribbon(data = plotF, aes(x = yearc, y = m, ymin = l, ymax = u, group = timex, fill = timex), alpha = 0.2, stat = "identity") +
  geom_errorbar(data = obsF, aes(x = yearc, y = m, ymin = l, ymax = u), color = 'black', width = 0, size = 0.6, ) +
  scale_x_continuous(breaks  = seq(1999, 2019, by = 5),  limits = c(1999, 2019),  
                     labels  = function(x){gsub(pattern = "1999", replacement = "1998/9", x = x)}) +
  geom_vline(aes(x = yearc, y = m), xintercept = 2009.5, linetype = 'dashed') +
  facet_wrap(strat ~ agegp, scales = 'free') + 
  #facet_grid2(strat ~ agegp, scales = "free", independent = "all", drop = TRUE) +
  theme_bw(base_size = 14, base_family = "American Typewriter") + 
  theme(axis.text.x = element_text(size = 14), axis.text.y = element_text(size = 10)) +
  labs(title = "", x = "Year", y = "Reported number of IPD cases per 1,000,000 population") +
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 1))  +
  theme(strip.text.x = element_text(size = 18), strip.background = element_rect(fill = "gray90")) +
  theme(legend.position = 'none')
...
ggsave(here::here("output", "POP", "sensitivity_pop.png"), 
       plot = sensitivity_pop, 
       width = 20, 
       height = 10, 
       unit = "in", 
       dpi = 300)
...
Model_logLik <-
  dplyr::bind_cols(
    logLik_efficacy <- rio::import(here::here('results', "POP", 'logLik_efficacy.csv')) %>% dplyr::rename('ll_efficacy' = 'XX.GOF_cases'),
    logLik_duration <- rio::import(here::here('results', "POP", 'logLik_duration.csv'))  %>% dplyr::rename('ll_duration' = 'XX.GOF_cases'),
    logLik_competition <- rio::import(here::here('results', "POP", 'logLik_competition.csv'))  %>% dplyr::rename('ll_competition' = 'XX.GOF_cases')) %>%
