...
lhs.sample.mt[,3] <- qunif(uniform_LHS[,3], min = 5, max = 15) #duration of protection of V - omega1
lhs.sample.mt[,4] <- qunif(uniform_LHS[,4], min = 5, max = 15) #duration of protection of F - omega2
...
lhs.sample.mt[,11] <- qunif(uniform_LHS[,11], min = 70, max = 120) #scale replacement parameter for 18+y

#total runs and number of simulations per run
total_run <- 1
len_each_run <- N_sample_size/total_run

rio::export(lhs.sample.mt, file = here("results", "POP", "lhs_parset.csv"))
