...
stan_data <- c(common_stan_data, list(
  scenario = 2L,
  #active parameter, bounds wide so the informative Gamma(3, 3) dominates.
  log_omega_F_13_lb = log(0.01),  
  log_omega_F_13_ub = log(5.00),
  #inactive declared parameters, bounds tight
  #do not enter dynamics in scenario 2 (variables wire to log_delta_F_7, log_rr_N_pre)
...
mod <- get_pcv13_model()
ipd_fit <- do.call(mod$sample, c(list(data = stan_data, seed = SEED_BASE + 2L, refresh = 50), mcmc_args))

ipd_fit$save_object(file = "ipd_fit_scenario2.rds")
...
print(ipd_fit$summary(c("omega_F_13")))
