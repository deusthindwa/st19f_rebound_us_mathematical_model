...
stan_data <- c(common_stan_data, list(
  scenario = 1L,
  #all three PCV13 parameters use the same wide brackets. In scenario 1
  #only log_delta_F_13 is active (informative Beta(3, 9) prior). The other
  ...
  #log_omega_F_7 / log_rr_N_pre). Wide brackets are cheap because the
...
#delta_F_13 at the Beta(3, 9) mean (= 3/12 = 0.25) on log scale.
init_fun <- make_init_fun(scenario = 1L, active_log_init = log(3 / 12))

mod <- get_pcv13_model()
ipd_fit_scen <- do.call(mod$sample, c(list(data = stan_data, seed = SEED_BASE + 1L, refresh = 50, init = init_fun), mcmc_args))

ipd_fit_scen$save_object(file = "ipd_fit_scenario1.rds")
...
print(ipd_fit_scen$summary(c("delta_F_13")))
