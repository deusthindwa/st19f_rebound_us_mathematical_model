...
1. `seed_state.rds` totals should match 1999 table populations (the rescaling step in the R burn-in enforces this).
2. `parameter_summary.csv` — R-hat < 1.01 and bulk ESS > a few hundred per chain for δ_V, δ_F, ω_V, ω_F. The `q_age` entries will have wider intervals (close to their priors). Inspect both `delta_*` and `log_delta_*` rows; if posteriors are pinned at the natural-scale boundaries (0.40 or 0.90), check that the chains are mixing on the log scale via `plot_01b_trace_log.png`.
3. `plot_06_obs_pred.png` — 1999 should sit exactly on its observed value, 2000 should be close (pre-vaccination dynamics), and 2001+ should be well covered by the 95 % bands. A dotted vertical line marks 2001 (the first year with vaccinated cohorts in the dynamics).
4. `plot_03b_nuisance_prior_posterior.png` — posteriors of `q_age` should be close to their log-uniform priors. `rr_*` no longer appear here because they are fixed data.
5. `plot_01_trace.png` and `plot_01b_trace_log.png` — chains should look like well-mixed white noise on both scales, with no chains stuck at boundaries.
