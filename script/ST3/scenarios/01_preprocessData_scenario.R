...
#pcv7 annual cases
ipd7 <-
  rio::import(here::here("data", "ST3", "usa_ipd.csv")) %>%
...
#pcv7A annual cases duering PCV13
ipd7A <-
  rio::import(here::here("data", "ST3", "usa_ipd.csv")) %>%
...
ipd13 <-
  ...
  dplyr::ungroup() %>%
  dplyr::mutate(agegp = factor(agegp, levels(factor(agegp))[c(1, 3, 4, 2)])) %>%
  mutate(stg = if_else(st %in% c('3', '4', '6A', '6B', '9V', '14', '18C', '23F', '1', '5', '6A', '7F', '19A'), "V", if_else(st == '19F', 'F', 'N')),
         age = if_else(agegp == '0-1y', 'gp1', if_else(agegp == '2-4y', 'gp2', if_else(agegp == '5-17y', 'gp3', 'gp4')))) %>%
  dplyr::group_by(yearc, age, stg) %>%
  dplyr::summarise(n = sum(n)) %>%
  dplyr::ungroup()
