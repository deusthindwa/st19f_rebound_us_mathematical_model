...
#bounds for uncertainty propagation the model draws q from Uniform(q_lower, q_upper) per MCMC iteration
q_lower <- c(`<1y` = 0.0125, `1-4y` = 0.0150, `5-17y` = 0.00864, `18+y` = 0.00517)
q_upper <- c(`<1y` = 0.0174, `1-4y` = 0.0185, `5-17y` = 0.0108, `18+y` = 0.00635)
q_mean <- (q_lower + q_upper) / 2 #used only for lightweight simulation R seed

#carriage clearance rates (per year)
carriage_dur_days <- c(71, 34, 18, 17)
gamma_age <- 365.25 / carriage_dur_days #per year (one value per age)
...
#relative risks of co-colonisation from carriage model
#bounds for uncertainty propagation (Stan samples each rr on log scale
#the mean is used only by the R seed burn-in to produce a near-steady-state seed state.
rr_V_lower <- 0.0712; rr_V_upper <- 0.198
rr_F_lower <- 0.352;  rr_F_upper <- 0.952
rr_N_lower <- 0.178;  rr_N_upper <- 0.524
rr_V <- (rr_V_lower + rr_V_upper) / 2 #midpoint for R seed burn-in only
rr_F <- (rr_F_lower + rr_F_upper) / 2
rr_N <- (rr_N_lower + rr_N_upper) / 2

#PCV7 coverage in infants (<1y)
vacc_cov_infants <- 0.86 # applied as part of birthing into <1y
...
#observed US population 1999-2009
...
pop_matrix2 <- as.matrix(us_pop2[, -1]) #rows = year, cols = age group
rownames(pop_matrix2) <- us_pop2$year

#observed IPD case counts
#rows = years 1999..2009, columns = age x serotype
...
obs_ipd_array <- function() {
  yrs <- obs_ipd$year
  arr <- array(NA_real_, dim = c(length(yrs), n_age, 3), dimnames = list(yrs, age_labels, c("V", "F", "N")))
...
#rows = years 1999..2019, columns = age x serotype
obs_ipd_ext <- tibble::tribble(
  ~year, ~V_1, ~V_2, ~V_3, ~V_4, ~F_1, ~F_2, ~F_3, ~F_4, ~N_1, ~N_2, ~N_3, ~N_4,
  1999,   597,  163,   61, 1510,   82,   24,    6,  107,   91,   28,   49, 1066,
  2000,   546,  155,   79, 1555,   61,   18,    6,  112,  103,   31,   37, 1069,
  2001,   208,  156,   64, 1433,   21,   37,    8,   96,  129,   45,   63, 1225,
  ...
  2008,     4,    6,    4,  226,    1,    2,    1,   23,  257,   58,  117, 2930,
  2009,     2,    2,    7,  159,    1,    2,    1,   26,  277,   80,  147, 2958,
  2010,   141,   96,   65, 1340,    1,    1,    1,   26,   89,   54,   40, 1599, #replaced F_3 with 1 for sanity of computing CCRs for post-PV13
...
#return array [n_year, n_age, n_serotype], serotype order = c("V","F","N")
obs_ipd_array_ext <- function() { 
  yrs  <- obs_ipd_ext$year
  arr  <- array(NA_real_, dim = c(length(yrs), n_age, 3), dimnames = list(yrs, age_labels, c("V", "F", "N")))
  cols <- c("V_1","V_2","V_3","V_4","F_1","F_2","F_3","F_4","N_1","N_2","N_3","N_4")
  m    <- as.matrix(obs_ipd_ext[, cols])
  for (k in seq_len(3)) {
    arr[, , k] <- m[, ((k - 1) * 4 + 1):(k * 4)]
  }
  arr
...
#bundle everything
#q_age, rr bounds (sampled in Stan for uncertainty propagation)
fixed_data <- list(
