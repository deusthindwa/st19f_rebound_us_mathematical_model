...
#multinomial likelihood given V, F, N and S states
  LL_prop=0
  for (age in 1:N_ages)
    LL_prop = LL_prop +
    calc_carr_logLik(c(df_EW$VT.prev[age], df_EW$NVT.prev[age], 1-(df_EW$VT.prev[age] + df_EW$NVT.prev[age])) * df_EW$N.carr[age],
                     c(df_EWm$V.prev.model[age], df_EWm$N.prev.model[age], 1-(df_EWm$V.prev.model[age] + df_EWm$F.prev.model[age] + df_EWm$N.prev.model[age]))
                     )
  return(LL_prop)
 }
...
fitLL <- optim(par = c(42.512, 1.5, 0.25, 0.25, rep(1, 4), rep(2.5, 4), rep(2.5, 4)),
               fn = fitmodel,
               dat = sim,
...
fitLL
exp(fitLL$par) # trans prob of VT
