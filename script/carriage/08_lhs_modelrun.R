...
#load R packages
source(here::here("script",'carriage', "00_main.R"))
#source("00_main.R")
...
#set up model including births, contacts, population
source(here::here("script", 'carriage', "02_setUp.R"))
#source("02_setUp.R")

#set up fixed parameter values
source(here::here("script", 'carriage', "03_modelParKnown.R"))
#source("03_modelParKnown.R")

#run model function
source(here::here("script", 'carriage', "04_transModel.R"))
#source("04_transModel.R")

# model fit using MAP (disabled)
source(here::here("script", 'carriage', "05_modelfit_MAP.R"))
#source("05_modelfit_MAP.R")

#call latin hypercube samples
source(here::here("script", 'carriage', "06_lhs_paraspace.R"))
#source("06_lhs_paraspace.R")
...
for( nn in 1:len_each_run){
  ...
  #parameter sets to run the dynamic model
  parmAdded <- list(baseV  = c(lhs.sample.mt[nn,1], lhs.sample.mt[nn,2], lhs.sample.mt[nn,3], lhs.sample.mt[nn,4]),
                    baseF  = c(lhs.sample.mt[nn,1], lhs.sample.mt[nn,2], lhs.sample.mt[nn,3], lhs.sample.mt[nn,4]),
                    baseN  = c(lhs.sample.mt[nn,1], lhs.sample.mt[nn,2], lhs.sample.mt[nn,3], lhs.sample.mt[nn,4]),
                    compV  = lhs.sample.mt[nn,5],
                    compF  = lhs.sample.mt[nn,6],
                    compN  = lhs.sample.mt[nn,7])
  ...
  #run model function with varied parameter values calling
  source(here::here("script", 'carriage', "07_lhs_model.R"))
  #source("07_lhs_model.R")
...
# Save all the runs together
rio::export(saved_carr_dynamics, here::here("results", 'carriage', "saved_carr_dynamics.rds"))
