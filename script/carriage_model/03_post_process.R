...
carriage_fit <- rio::import(here::here("results", "carriage_model", "carriage_fit.rds"))


#posterior diagnostics
key_pars <- c("log_rho_age", "rho_age", "eps_V", "eps_F", "eps_N", "rho_V", "rho_F", "rho_N", "lambda_V_eq", "lambda_F_eq", "lambda_N_eq")
param_names <- c(paste0("rho_age[", 1:n_age, "]"), paste0("eps_V"), paste0("eps_F"), paste0("eps_N"))
...
#trace plots
trace_pars <- c("log_rho_age[1]","log_rho_age[2]","log_rho_age[3]","log_rho_age[4]", "eps_V","eps_F","eps_N")
trace_plot <- bayesplot::mcmc_trace(carriage_fit$draws(trace_pars))

print(trace_plot)
ggsave(here::here("output", "carriage_model", "trace_plot.png"), trace_plot, width = 9, height = 6, dpi = 330, bg = "white")
...
p_pairs <- 
  bayesplot::mcmc_pairs(draws_array, pars = c("rho_age[1]","rho_age[2]","rho_age[3]","rho_age[4]", "eps_V","eps_F","eps_N"), 
...
eps_draws <- carriage_fit$draws(c("eps_V","eps_F","eps_N"), format = "draws_matrix")
...
df_obs <- 
  ...
  dplyr::mutate(n = S + V + F + N,
                ...
                F_L = wald_ci(F, n)$L, F_U = wald_ci(F, n)$U,
                N_L = wald_ci(N, n)$L, N_U = wald_ci(N, n)$U) %>%
  dplyr::mutate(n = S + V + F + N) %>%
  tidyr::pivot_longer(cols = c(S, V, F, N), names_to = "state", values_to = "count") %>%
...
pp_summary <- tidyr::expand_grid(age = 1:n_age, k = 1:4) %>%
  dplyr::mutate(
    ...
    state    = factor(c("S","V","F","N")[k], levels = c("S","V","F","N")),
...
plot_df1 <- 
  ...
  dplyr::filter(state %in% c('S', "F", "V", "N")) %>%
  dplyr::mutate(state = factor(state,   levels = c('S', "F", "V", "N")),
...
ppc_prop_plot <-
  ...
  facet_wrap(factor(state, levels=c('S', 'F', 'V', 'N')) ~ ., scales = 'free', ncol = 4) +
...
#rho_V = rho_F = rho_N = rho_age, so the three lines overlap;
rho_long <- bind_rows(carriage_fit$draws("rho_V", format = "draws_df") %>% as_tibble() %>%
                      ...
                      carriage_fit$draws("rho_F", format = "draws_df") %>% as_tibble() %>%
                        tidyr::pivot_longer(starts_with("rho_F"), names_to = "var", values_to = "rho") %>%
                        dplyr::mutate(serotype = "F", age = as.integer(sub("rho_F\\[(\\d+)\\]", "\\1", var))),
...
#compartment composition per age group (S, V, F, N, VF, NV, NF) at equilibrium
comp_names <- c("S","V","F","N","VF","NV","NF")
...
comp_bar_plot <-
  ...
  labs(x = "Age group", y = "Equilibrium proportion", fill = "Compartment", title = "Equilibrium compartment composition (posterior mean)", subtitle = "S, V, F, N (single), VF / NV / NF (dual carriage)")
...
#equilibrium force of infection (per year) lambda_V_eq, lambda_F_eq, lambda_N_eq
lambda_draws <-
  carriage_fit$draws(c("lambda_V_eq","lambda_F_eq","lambda_N_eq"), format = "draws_df") %>% as_tibble()
