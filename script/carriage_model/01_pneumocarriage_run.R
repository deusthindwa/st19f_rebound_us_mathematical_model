...
# a deterministic age-structured SIS model of pneumococcal carriage (V, F, N serotype groups across 4 age groups).
#
...
#
# eps_V, eps_F, eps_N, 3 serotype-specific competition
...
#eps_V, eps_F, eps_N are estimated in Stan

#observed counts: columns S (non-carriers), V, F, N
y_obs <- matrix(c(
...
sis_rhs <- function(t, y, parms) {
  with(parms, {
    ...
    sumV <- as.numeric(C %*% (V  + NV + VF))
    sumF <- as.numeric(C %*% (Fc + NF + VF))
    sumN <- as.numeric(C %*% (Nc + NV + NF))
    ...
    dy <- numeric(7 * n)
    for (i in 1:n) {
      rVi <- r_V[i]; rFi <- r_F[i]; rNi <- r_N[i]; li <- l_age[i]

      dS  <- -(lamV[i] + lamF[i] + lamN[i]) * S[i] +
              rVi * V[i] + rFi * Fc[i] + rNi * Nc[i]
      dV  <-  lamV[i] * S[i] - rVi * V[i] -
              eps_V * (lamF[i] + lamN[i]) * V[i] +
              rFi * VF[i] + rNi * NV[i]
      dFc <-  lamF[i] * S[i] - rFi * Fc[i] -
              eps_F * (lamV[i] + lamN[i]) * Fc[i] +
              rVi * VF[i] + rNi * NF[i]
      dNc <-  lamN[i] * S[i] - rNi * Nc[i] -
              eps_N * (lamV[i] + lamF[i]) * Nc[i] +
              rVi * NV[i] + rFi * NF[i]
      dVF <-  eps_V * lamF[i] * V[i] +
              eps_F * lamV[i] * Fc[i] - (rVi + rFi) * VF[i]
      dNV <-  eps_V * lamN[i] * V[i] +
              eps_N * lamV[i] * Nc[i] - (rVi + rNi) * NV[i]
      dNF <-  eps_F * lamN[i] * Fc[i] +
              eps_N * lamF[i] * Nc[i] - (rFi + rNi) * NF[i]

      if (i == 1) {
        dS  <- dS  + li * (p_birth[1] - S[i])
        dV  <- dV  + li * (p_birth[2] - V[i])
        dFc <- dFc + li * (p_birth[3] - Fc[i])
        dNc <- dNc + li * (p_birth[4] - Nc[i])
        dVF <- dVF + li * (p_birth[5] - VF[i])
        dNV <- dNV + li * (p_birth[6] - NV[i])
        dNF <- dNF + li * (p_birth[7] - NF[i])
      } else {
        dS  <- dS  + li * (S[i - 1]  - S[i])
        dV  <- dV  + li * (V[i - 1]  - V[i])
        dFc <- dFc + li * (Fc[i - 1] - Fc[i])
        dNc <- dNc + li * (Nc[i - 1] - Nc[i])
        dVF <- dVF + li * (VF[i - 1] - VF[i])
        dNV <- dNV + li * (NV[i - 1] - NV[i])
        dNF <- dNF + li * (NF[i - 1] - NF[i])
      }
...
parms0 <- list(
  n_age   = n_age,
  C       = C_year,
  r_V     = r_V,  r_F = r_F,  r_N = r_N,
  l_age   = l_age,
  p_birth = p_birth,
  rho_V   = rho_age_default,
  rho_F   = rho_age_default,
...
predicted_prev <- vapply(1:n_age, function(i) {
  ...
  NFi <- final[(i - 1) * 7 + 7]
  c(S = Si,
    V = Vi + 0.5 * VFi + 0.5 * NVi,
    F = Fi + 0.5 * VFi + 0.5 * NFi,
    N = Ni + 0.5 * NVi + 0.5 * NFi)
}, numeric(4))

predicted_prev <- t(predicted_prev)
rownames(predicted_prev) <- age_groups
colnames(predicted_prev) <- c("S", "V", "F", "N")
...
#compile stan model
#spn_model <- cmdstanr::cmdstan_model(here::here("script", "carriage_model", "02_pneumocarriage_model.stan"))
...
# carriage_fit$save_object(file = "carriage_fit.rds")
